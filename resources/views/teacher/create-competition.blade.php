...
<div class="max-w-3xl mx-auto px-4 sm:px-6">
    ...
    <div class="mb-7 sm:mb-10">
        <h1 class="text-2xl sm:text-4xl font-bold text-slate-900 tracking-tight">Create Competition</h1>
        <p class="text-slate-500 mt-2 text-sm">Set up a new competition for students to participate in.</p>
    </div>
    ...
    <div id="competitionForm" class="bg-white border border-slate-200 rounded-3xl shadow-xl p-6 sm:p-8">
        ...
        <form method="POST" action="{{ route('teacher.competitions.store') }}" class="space-y-6 sm:space-y-8">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}"
                       ...
                       focus:ring-2 focus:ring-slate-900 focus:outline-none" required>
                @error('title') <p class="text-sm text-rose-600 mt-2">{{ $message }}</p> @enderror
            </div>
            ...
            <div>
                ...
                @error('description') <p class="text-sm text-rose-600 mt-2">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Rules & Guidelines</label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Competition Start Date</label>
                    <input type="datetime-local" name="start_date" value="{{ old('start_date') }}"
                           class="w-full px-5 py-3 rounded-2xl border border-slate-300 focus:ring-2 focus:ring-slate-900 focus:outline-none text-sm" required>
                    @error('start_date') <p class="text-sm text-rose-600 mt-2">{{ $message }}</p> @enderror
                </div>
                <div>
                    ...
                    <input type="datetime-local" name="deadline" value="{{ old('deadline') }}"
                           class="w-full px-5 py-3 rounded-2xl border border-slate-300 focus:ring-2 focus:ring-slate-900 focus:outline-none text-sm" required>
                    @error('deadline') <p class="text-sm text-rose-600 mt-2">{{ $message }}</p> @enderror
                </div>
            </div>

            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Entry Fee (INR)</label>
                <div class="relative">
                    <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-medium text-sm">₹</span>
                    <input type="number" name="fee" step="0.01" min="0" value="{{ old('fee') }}"
                           placeholder="0.00"
                ...
                @error('fee') <p class="text-sm text-rose-600 mt-2">{{ $message }}</p> @enderror
            </div>
